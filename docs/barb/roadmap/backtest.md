...
### Phase 7b: Interactive Strategy Card

Checkpoint перед запуском бэктеста. Модель генерирует `run_backtest` tool call → фронтенд показывает карточку с параметрами → пользователь проверяет, подкручивает если нужно → "Запустить" → движок считает → результат кидается модели для анализа.

**Проблема сейчас:**

Пользователь просит тест → модель сразу бежит считать. Если что-то не так (забыл сессию, хочет другой стоп, период не тот) — пишет ещё сообщение → полный цикл токенов заново. Каждая коррекция = весь контекст чата в модель + новый tool call + новый результат + новый анализ. Чем длиннее разговор, тем дороже каждая правка.

**Что даёт карточка:**

1. **Checkpoint** — пользователь видит что модель поняла ДО запуска. Ловит ошибки до того как сожжены токены
2. **Прямые правки** — поменять стоп с 2% на 1.5% = клик, не сообщение. 0 токенов на "понять что поменять"
3. **Ощущение контроля** — пользователь видит все параметры и управляет процессом, а не ждёт что AI сделает

**Экономия токенов:**

```
Коррекция без карточки:
  Контекст чата (растёт!) → [LLM: парсит] → [LLM: tool call] → engine → [LLM: анализ]
  ~20K+ токенов на одно "а я RTH хотел"

Коррекция с карточкой:
  Клик в форме → engine → [LLM: анализ]
  ~5K токенов (только анализ результата)
```

30-50% бэктест-разговоров включают хотя бы одну коррекцию параметров.

**Флоу:**
```
1. "Протестируй лонг после 3 красных дней, стоп 2%, тейк 3%"
2. Модель генерирует run_backtest tool call с:
   - entry: "prev(close) < prev(open) and ..."  (для движка)
   - entry_label: "3 красных дня подряд"         (для карточки)
3. Фронтенд рендерит StrategyCard:
   ┌ 3 красных дня подряд ────────────┐
   │  Направление: [Long ▾]           │
   │  Стоп:        [2%]               │
   │  Тейк:        [3%]               │
   │  Трейлинг:    [—]                │
   ...
   │  [Запустить]  [Отмена]           │
   └──────────────────────────────────┘
4. Пользователь проверяет / подкручивает → "Запустить"
5. Фронтенд вызывает POST /api/backtest напрямую (без LLM)
6. Результат отправляется модели → модель анализирует как обычно
```

**Entry: два поля**
- `entry` — Barb Script выражение для движка (как сейчас)
- `entry_label` — человеческий текст для карточки (модель переводит обратно, одно поле в схеме)
- entry_label = заголовок карточки (read-only). Выражение скрыто за "▸ код"
- Хочешь поменять логику входа — пиши в чат, это задача модели

**Архитектура:**
- StrategyCard — свой компонент. Состояния: editing → running → result
- Форма динамическая: рендерит поля из tool call JSON. Модель прислала trailing_stop → поле есть. Не прислала → "Добавить параметр"
- `POST /api/backtest` — прямой вызов движка, возвращает BacktestResult
- Результат инжектится в чат как tool result → модель комментирует
- "Отмена" → модель получает отказ, может спросить что поменять

**Позже — вариации:**
- Кнопка "+ Вариация" клонирует карточку с другими параметрами
- Табы сверху правой панели: Test 1, Test 2, Test 3
- Сравнение результатов нескольких тестов рядом
...
См. `backtest-v3-fills.md` для деталей. `slippage_atr` и position sizing отложены — для 1 контракта фиксированный slippage достаточен, дефолты решаются через интерактивную карточку (Phase 7b).
